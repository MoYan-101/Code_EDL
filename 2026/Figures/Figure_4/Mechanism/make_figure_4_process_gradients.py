...
HEATMAP_INPUTS_DIR = ROOT / "Figures" / "Figure_4" / "Heatmap" / "inputs"
...
PNG_OUT = OUT_DIR / f"figure_4_process_gradients_{OUTPUT_TAG}.png"
SVG_OUT = OUT_DIR / f"figure_4_process_gradients_{OUTPUT_TAG}.svg"
CSV_OUT = CSV_DIR / f"figure_4_process_sweeps_{OUTPUT_TAG}.csv"
SCHEMATIC_CSV_OUT = CSV_DIR / f"figure_4_polarization_schematic_{OUTPUT_TAG}.csv"
CURVE_CSV_OUT = CSV_DIR / f"figure_4_polarization_curves_{OUTPUT_TAG}.csv"
...
COLUMN_FIGURE_STEMS = {
    "Cdl_Au": "figure_4_column_cdliau_causal_chain",
    "Cdl_Pd": "figure_4_column_cdlpd_causal_chain",
    "pzc_Au": "figure_4_column_pzcau_causal_chain",
    "pzc_Pd": "figure_4_column_pzcpd_causal_chain",
}

COLUMN_FIGURE_TITLES = {
    "Cdl_Au": r"Figure 4 column: $C_{\mathrm{dl,Au}}$ causal chain",
    "Cdl_Pd": r"Figure 4 column: $C_{\mathrm{dl,Pd}}$ causal chain",
    "pzc_Au": r"Figure 4 column: $\mathrm{PZC}_{\mathrm{Au}}$ causal chain",
    "pzc_Pd": r"Figure 4 column: $\mathrm{PZC}_{\mathrm{Pd}}$ causal chain",
}
...
def plot_process_gradients(rows: list[dict[str, Any]]) -> None:
    ...
    fig.suptitle(
        "Figure 4 mechanism: parameter changes propagate through EDL charging to mixed-potential outputs",
        x=0.02,
        y=0.992,
        ha="left",
        va="top",
    ...
    fig.text(
        0.02,
        0.966,
        "Each column varies one parameter only. Dashed horizontal lines mark the Figure 4 baseline; dashed vertical lines in the bottom rows mark the no-EDL output.",
        ha="left",
        va="top",
        fontsize=7.0,
        color=COLORS["gray"],
